...
public:
    ...
    explicit inline shared_lock_guard(Mutex &m) : mutex_(&m) { mutex_->lock_sharable(); }

    inline ~shared_lock_guard() { if (mutex_) mutex_->unlock_sharable(); }
...
public:
    ...
    explicit inline optional_shared_lock_guard(Mutex* m) : mutex_(m) { if (mutex_) mutex_->lock_sharable(); }

    inline ~optional_shared_lock_guard() { if (mutex_) mutex_->unlock_sharable(); }
