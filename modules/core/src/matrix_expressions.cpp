...
public:
    ...
    void assign(const MatExpr& expr, Mat& m, ElemType type = CV_TYPE_UNSPECIFIED) const CV_OVERRIDE;
...
public:
    ...
    static void makeExpr(MatExpr& res, int method, Size sz, ElemType type, double alpha = 1);
    static void makeExpr(MatExpr& res, int method, int ndims, const int* sizes, ElemType type, double alpha = 1);
};
...
ElemType MatOp::type(const MatExpr& expr) const
{
    CV_INSTRUMENT_REGION()
...
ElemType MatExpr::type() const
{
    ...
    if( isCmp(*this) )
        return CV_8UC1;
    return op ? op->type(*this) : CV_TYPE_UNSPECIFIED;
}


/////////////////////////////////////////////////////////////////////////////////////////////////////

void MatOp_Identity::assign(const MatExpr& e, Mat& m, ElemType _type) const
{
    if (_type == CV_TYPE_UNSPECIFIED)
        _type = e.a.type();
    ...
    else
    {
        ...
        e.a.convertTo(m, CV_MAT_DEPTH(_type));
    }
}
...
void MatOp_AddEx::assign(const MatExpr& e, Mat& m, ElemType _type) const
{
    if (_type == CV_TYPE_UNSPECIFIED)
        _type = e.a.type();

    Mat temp, &dst = e.a.type() == _type ? m : temp;
    if( e.b.data )
    {
    ...
    {
        e.a.convertTo(m, CV_MAT_DEPTH(_type), e.alpha, e.s[0]);
        return;
    }
    else if( e.alpha == 1 )
        cv::add(e.a, e.s, dst);
...
void MatOp_Bin::assign(const MatExpr& e, Mat& m, ElemType _type) const
{
    if (_type == CV_TYPE_UNSPECIFIED)
        _type = e.a.type();
    ...
    if( dst.data != m.data )
        dst.convertTo(m, CV_MAT_DEPTH(_type));
}
...
void MatOp_Cmp::assign(const MatExpr& e, Mat& m, ElemType _type) const
{
    if (_type == CV_TYPE_UNSPECIFIED)
        _type = e.a.type();

    //TODO: Investigate why CV_8U is coded here
    Mat temp, &dst = _type == e.a.type() || _type == CV_8U ? m : temp;
...
void MatOp_T::assign(const MatExpr& e, Mat& m, ElemType _type) const
{
    if (_type == CV_TYPE_UNSPECIFIED)
        _type = e.a.type();

    Mat temp, &dst = _type == e.a.type() ? m : temp;

    cv::transpose(e.a, dst);

    if( dst.data != m.data || e.alpha != 1 )
        dst.convertTo(m, CV_MAT_DEPTH(_type), e.alpha);
}
...
void MatOp_GEMM::assign(const MatExpr& e, Mat& m, ElemType _type) const
{
    if (_type == CV_TYPE_UNSPECIFIED)
        _type = e.a.type();
...
void MatOp_Invert::assign(const MatExpr& e, Mat& m, ElemType _type) const
{
    if (_type == CV_TYPE_UNSPECIFIED)
        _type = e.a.type();
...
void MatOp_Solve::assign(const MatExpr& e, Mat& m, ElemType _type) const
{
    if (_type == CV_TYPE_UNSPECIFIED)
        _type = e.a.type();
...
void MatOp_Initializer::assign(const MatExpr& e, Mat& m, ElemType _type) const
{
    if (_type == CV_TYPE_UNSPECIFIED)
        _type = e.a.type();
...
inline void MatOp_Initializer::makeExpr(MatExpr& res, int method, Size sz, ElemType type, double alpha)
{
    res = MatExpr(getGlobalMatOpInitializer(), method, Mat(sz, type, (void*)(size_t)0xEEEEEEEE), Mat(), Mat(), alpha, 0);
}

inline void MatOp_Initializer::makeExpr(MatExpr& res, int method, int ndims, const int* sizes, ElemType type, double alpha)
{
...
MatExpr Mat::zeros(int rows, int cols, ElemType type)
{
    CV_INSTRUMENT_REGION()
...
MatExpr Mat::zeros(Size size, ElemType type)
{
    CV_INSTRUMENT_REGION()
...
MatExpr Mat::zeros(int ndims, const int* sizes, ElemType type)
{
    CV_INSTRUMENT_REGION()
...
MatExpr Mat::ones(int rows, int cols, ElemType type)
{
    CV_INSTRUMENT_REGION()
...
MatExpr Mat::ones(Size size, ElemType type)
{
    CV_INSTRUMENT_REGION()
...
MatExpr Mat::ones(int ndims, const int* sizes, ElemType type)
{
    CV_INSTRUMENT_REGION()
...
MatExpr Mat::eye(int rows, int cols, ElemType type)
{
    CV_INSTRUMENT_REGION()
...
MatExpr Mat::eye(Size size, ElemType type)
{
    CV_INSTRUMENT_REGION()
