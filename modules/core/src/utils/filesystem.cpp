...
bool isExist(const cv::String& path)
{
...
void FileLock::lock_sharable() { CV_Assert(pImpl->lock_sharable()); }
void FileLock::unlock_sharable() { CV_Assert(pImpl->unlock_sharable()); }



cv::String getOpenCVCacheDirectory(const char* sub_directory_name, const char* configuration_name)
{
    String cache_path;
