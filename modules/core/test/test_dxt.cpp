                                                         ...
                                                         vector<vector<ElemType> >& types )
{
    ...
    if( spectrum_mode )
    {
        if( cn == 1 )
        {
            types[OUTPUT][0] = static_cast<ElemType>(depth + 8);
            sizes[TEMP][0] = size;
        }
    ...
    {
        types[TEMP][0] = static_cast<ElemType>(depth + 8); // CV_??FC2
        sizes[TEMP][0] = size;
...
protected:
    void run(int)
    {
        ...
        for( int i = 0; i < 10; i++ )
        {
            ...
            ElemDepth depth = static_cast<ElemDepth>(rng.uniform(0, 2) + CV_32F);
